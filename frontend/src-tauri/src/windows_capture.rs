    ...
    unsafe {
        ...
        for info in &capture_infos {
                                        ...
                                        if hr.is_ok() {
                                            if let Some(staging) = staging_opt {
                                                ...
                                                if map_hr.is_ok() {
                                                    ...
                                                    let width_bytes = (desc.Width * 4) as usize;
                                                    let mut rgb_buf = vec![0u8; (desc.Width * desc.Height * 4) as usize];
                                                    let data_ptr = mapped.pData as *const u8;
                                                    
                                                    for r in 0..desc.Height as usize {
                                                        let src_offset = r * pitch;
                                                        let dst_offset = r * width_bytes;
                                                        
                                                        // Fast row copy skipping padding
                                                        std::ptr::copy_nonoverlapping(
                                                            data_ptr.add(src_offset),
                                                            rgb_buf.as_mut_ptr().add(dst_offset),
                                                            width_bytes // Exact valid bytes per row
                                                        );
                                                    }
            ...
            if let Some((rgb_data, fw, fh)) = captured_frame {
                // `rgb_data` currently contains width * height * 4 bytes (BGRA).
                // Convert back to dense RGB since JPEG encoder requires it and stitching requires it.
                // We do this here after the fast RowPitch copy above to avoid slow pixel-by-pixel extraction during the GPU map.
                let mut dense_rgb = Vec::with_capacity((fw * fh * 3) as usize);
                for r in 0..fh as usize {
                    let row_start = r * (fw * 4) as usize; // Stride is now exactly fw * 4, GPU padding stripped.
                    for c in 0..fw as usize {
                        let px = row_start + (c * 4);
                        if px + 2 < rgb_data.len() {
                            let b = rgb_data[px];
                            let g = rgb_data[px + 1];
                            let r_val = rgb_data[px + 2];
                            dense_rgb.push(r_val);
                            dense_rgb.push(g);
                            dense_rgb.push(b);
                        }
                    }
                ...
                if capture_infos.len() == 1 {
                    ...
                    if dense_rgb.len() >= 3 {
                        let first_r = dense_rgb[0];
                        let first_g = dense_rgb[1];
                        let first_b = dense_rgb[2];
                        for i in (0..dense_rgb.len()).step_by(3) {
                            if dense_rgb[i] != first_r || dense_rgb[i+1] != first_g || dense_rgb[i+2] != first_b {
                                is_blank = false;
                                break;
                            }
                        }
                    ...
                    encoder
                        .encode(&dense_rgb, fw, fh, image::ColorType::Rgb8.into())
                ...
                let monitor_stride = fw as usize; // Now we use dense_rgb, stride is strictly width
                ...
                for r in 0..fh as usize {
                    ...
                    for c in 0..fw as usize {
                        ...
                        if d_idx + 2 < full_desktop_rgb.len() && s_idx + 2 < dense_rgb.len() {
                            full_desktop_rgb[d_idx] = dense_rgb[s_idx];
                            full_desktop_rgb[d_idx + 1] = dense_rgb[s_idx + 1];
                            full_desktop_rgb[d_idx + 2] = dense_rgb[s_idx + 2];
                        }
                    }
                }
                any_success = true;
